...
<body class="text-gray-800">
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mt-5 mb-5 no-print flex justify-between items-center">
        ...
        <button onclick="window.print()"
            class="bg-primary hover:bg-blue-800 text-white px-5 py-2 rounded-lg font-medium transition flex items-center shadow">
    ...
    <div class="invoice-box shadow-sm">
        ...
        <div class="flex justify-between items-start border-b border-gray-200 pb-8 mb-8">
            ...
            <div class="text-right">
                <h1 class="text-3xl font-bold {{ $order->status === 'cancelled' ? 'text-red-500 line-through' : 'text-dark' }} mb-1">INVOICE</h1>
                <p class="text-gray-500 font-mono">#{{ $order->order_code }}</p>
                <span class="mt-2 inline-flex items-center px-2.5 py-1 rounded text-xs font-bold uppercase tracking-wide {{ $order->status === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700' }}">
                    {{ $order->status_label }}
                </span>
            </div>
        </div>
        ...
        <div class="flex flex-col md:flex-row justify-between mb-8 gap-8">
            <div>
                ...
                <p class="text-gray-600 text-sm"><i class="fa-solid fa-envelope text-gray-400 mr-1 w-4"></i>
                    {{ $order->user->email }}</p>
                @if($order->user->phone)
                    <p class="text-gray-600 text-sm"><i class="fa-solid fa-phone text-gray-400 mr-1 w-4"></i>
                        {{ $order->user->phone }}</p>
                @endif
            </div>
            <div class="md:text-right">
                ...
                <div class="grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-right md:text-right">
                    ...
                    <span
                        class="text-dark font-medium">{{ $order->payment_method === 'cod' ? 'COD (Bayar di Tempat)' : 'Transfer Bank' }}</span>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b-2 border-gray-200">
                        ...
                        <th class="py-3 text-sm font-bold text-gray-700 uppercase tracking-wide text-right">Harga Satuan
                        </th>
                        <th class="py-3 text-sm font-bold text-gray-700 uppercase tracking-wide text-right">Subtotal
                        </th>
                    </tr>
                </thead>
                <tbody class="text-sm">
                    @foreach($order->items as $item)
                        <tr class="border-b border-gray-100">
                            ...
                            <td class="py-4 text-right font-semibold text-dark">Rp
        ...
        <div class="flex flex-col md:flex-row justify-between items-start">
            <div class="mb-6 md:mb-0 max-w-sm">
                @if($order->notes)
                    ...
                    <p class="text-sm text-gray-600 bg-gray-50 p-3 rounded border border-gray-100 italic">
                        {{ $order->notes }}</p>
                @endif
            </div>
            ...
            <div class="w-full md:w-1/2 lg:w-1/3">
                <div class="space-y-3 text-sm">
                    ...
                    <div class="flex justify-between items-center pt-1">
                        ...
                        <span class="text-xl font-bold {{ $order->status === 'cancelled' ? 'text-gray-400 line-through' : 'text-primary' }}">Rp
                            {{ number_format((float) $order->grand_total, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="mt-8 border-t border-dashed border-red-200 pt-6">
            <div class="bg-red-50 text-red-600 p-4 rounded-lg flex items-center gap-3">
                <i class="fa-solid fa-triangle-exclamation text-2xl"></i>
                <div>
                    <p class="font-bold">INVOICE TIDAK BERLAKU (VOID)</p>
                    <p class="text-sm">Pesanan ini telah dibatalkan. Tagihan ini tidak lagi valid dan kewajiban pembayaran telah ditiadakan.</p>
                </div>
            </div>
        </div>
        @endif
