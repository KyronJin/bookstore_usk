            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <form action="{{ route('katalog') }}" method="GET" class="flex gap-2 w-full md:w-auto">
                        ...
                        @endif
                        <div
                            class="flex items-center bg-white/15 backdrop-blur-sm border border-white/30 rounded-xl px-4 py-2.5 flex-grow md:w-72 focus-within:bg-white/25 transition">
                        ...
                        <button type="submit"
                            class="bg-accent hover:bg-yellow-500 text-white px-5 py-2.5 rounded-xl font-medium transition text-sm whitespace-nowrap">
                            Cari
                        </button>
                    </form>
                </div>
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
                        <div>
                            <p class="text-gray-600 text-sm">
                                Menampilkan <span
                                    class="font-bold text-dark">{{ $books->firstItem() ?? 0 }}–{{ $books->lastItem() ?? 0 }}</span>
                        ...
                        <div class="sm:hidden w-full">
                            <select onchange="window.location=this.value"
                                class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-secondary">
                                @foreach(['terbaru' => 'Terbaru', 'az' => 'Judul A-Z', 'za' => 'Judul Z-A', 'harga_terendah' => 'Harga Terendah', 'harga_tertinggi' => 'Harga Tertinggi'] as $val => $label)
                                    <option
                                        value="{{ route('katalog', array_merge(request()->except('sort', 'page'), ['sort' => $val])) }}"
                                        {{ $sort === $val ? 'selected' : '' }}>
                    ...
                    @if($books->isEmpty())
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 py-20 text-center">
                            ...
                            <a href="{{ route('katalog') }}"
                                class="inline-block bg-secondary text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-blue-600 transition">
                                Lihat Semua Buku
                            </a>
                        </div>
                    @else
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
                            @foreach($books as $book)
                                <div
                                    class="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden flex flex-col group border border-gray-100 hover:-translate-y-1">
                                    <!-- Gambar -->
                                    <div class="relative overflow-hidden bg-gray-100" style="height:220px">
                                        ...
                                        <div class="absolute top-2 left-2 flex flex-col gap-1">
                                            @if($book->is_featured)
                                                <span class="bg-accent text-white text-xs font-bold px-2 py-0.5 rounded-full shadow">⭐
                                                    Pilihan</span>
                                            @endif
                                        </div>
                                        ...
                                        @if($book->stock <= 0)
                                            <div class="absolute inset-0 bg-black/50 flex items-center justify-center">
                                                <span class="bg-red-500 text-white text-sm font-bold px-3 py-1 rounded-full shadow">Stok
                                                    Habis</span>
                                            </div>
                                        @elseif($book->stock <= 5)
                                            <div class="absolute bottom-2 left-2">
                                                <span
                                                    class="bg-orange-500 text-white text-xs font-bold px-2 py-0.5 rounded-full shadow">Sisa
                                                    {{ $book->stock }}</span>
                                            </div>
                                        @endif
                                    </div>
                                    ...
                                    <div class="p-4 flex flex-col flex-grow">
                                        ...
                                        <h3 class="text-sm font-bold text-dark mb-0.5 line-clamp-2 leading-snug">{{ $book->title }}
                                        </h3>
                                        ...
                                        <div class="mt-auto flex items-center justify-between pt-3 border-t border-gray-50">
                                            <span class="text-lg font-bold text-primary">Rp
                                                {{ number_format((float) $book->price, 0, ',', '.') }}</span>
                                            ...
                                            @else
                                                <button disabled
                                                    class="bg-gray-200 text-gray-400 h-9 px-4 rounded-lg text-xs font-semibold cursor-not-allowed">
                                                    Habis
                                                </button>
                                            @endif
                                        </div>
